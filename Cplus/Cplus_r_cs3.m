    ...
    while ctr lt counter do
        ...
        if not ([a, c] in tested_pmts) then
            ...
            if (Gcd(Gcd(a, bp), c) eq 1) and (a*bp*c ne 0) and (a mod r eq 0) then //assuming that the solution to the dioph. eq. is nontrivial
                ...
                depth_m := p*Valuation(a, r)/2 - r/(r-1);
                print "\nExpected depths of the twins:", depth_m + 2/(r-1), ", ", 2*depth_m + 2/(r-1), ", outer depth:", 2/(r-1), "\n";
...
print "Cplus(r, p, a, c) at r when r divides a .";
print "##########################################################\n\n";
...
print "When r = 5, p = 7, we expect the conductor exponent at r to be 3. We find:\n";
print res_5_7;
print "\nWhen r = 5, p = 11, we expect the conductor exponent at r to be 3. We find:\n";
print res_5_11;
print "\nWhen r = 7, p = 5, we expect the conductor exponent at r to be 5. We find:\n";
print res_7_5;
print "\nWhen r = 7, p = 11, we expect the conductor exponent at r to be 5. We find:\n";
print res_7_11;